    ...
    <>
      ...
      <nav className={`navbar navbar-vertical${scrolled ? ' scrolled' : ''}`}>
        ...
        <ul className={`navbar-links navbar-links-center${menuOpen ? ' show' : ''}`}>
          ...
          {!user && (
            <li className="mobile-menu-item">
              <button className="mobile-login-btn" onClick={() => { onLoginClick(); closeMenu(); }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{marginRight: '0.5rem', verticalAlign: 'middle'}}>
          ...
          {user && (
            <li className="mobile-menu-item mobile-user-info">
              <div className="mobile-user-header">
                {user.photoURL ? (
                  <img src={user.photoURL} alt="Profile" className="mobile-user-avatar" />
                ) : (
              ...
              <button className="mobile-menu-link mobile-logout" onClick={() => { handleLogout(); closeMenu(); }}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{marginRight: '0.5rem', verticalAlign: 'middle'}}>
                  <path d="M6 14H2V2H6V0H2C0.9 0 0 0.9 0 2V14C0 15.1 0.9 16 2 16H6V14ZM12.09 11.59L10.67 10.17L12.84 8H5V6H12.84L10.66 3.83L12.08 2.41L16.67 7L12.09 11.59Z" fill="currentColor"/>
                </svg>
                Logout
